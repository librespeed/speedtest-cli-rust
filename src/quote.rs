//! Go's `%q` rendering of a byte string.
//!
//! `--debug` shows a probe or getIP answer that did not parse the way the Go
//! client does, so the two clients' debug output can be compared line for
//! line. Quoting escapes what would drive the terminal or forge a line, and
//! unlike `output::sanitize` it keeps every byte recoverable, including those
//! that are not UTF-8.

use std::fmt::{self, Write};

/// Renders bytes as Go's `%q` verb renders a `[]byte`, which is
/// `strconv.Quote` of the bytes as a string.
///
/// A Display wrapper rather than a function returning a String, so a debug
/// line that is not printed does no work.
pub struct GoQuote<'a>(pub &'a [u8]);
...
impl fmt::Display for GoQuote<'_> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        for chunk in self.0.utf8_chunks() {
            ...
            // Go decodes one byte at a time and escapes each byte that does
            // not start a valid sequence. The bytes of an invalid chunk after
            // its first are continuation bytes, which never start one, so
            // escaping them all gives the same result.
...
/// Writes one character as `strconv.Quote` does.
fn write_escaped(f: &mut fmt::Formatter<'_>, c: char) -> fmt::Result {
    match c {
        '"' | '\\' => write!(f, "\\{c}"),
        c if is_print(c) => f.write_char(c),
        '\x07' => f.write_str("\\a"),
        '\x08' => f.write_str("\\b"),
        '\x0c' => f.write_str("\\f"),
        '\n' => f.write_str("\\n"),
        ...
        '\t' => f.write_str("\\t"),
        '\x0b' => f.write_str("\\v"),
        c if c < ' ' || c == '\x7f' => write!(f, "\\x{:02x}", c as u32),
        c if (c as u32) < 0x10000 => write!(f, "\\u{:04x}", c as u32),
        c => write!(f, "\\U{:08x}", c as u32),
    }
}

/// Whether Go's `strconv.IsPrint` accepts a character: a letter, mark,
/// number, punctuation or symbol, or the ASCII space.
fn is_print(c: char) -> bool {
    let c = c as u32;
    let i = GO_PRINTABLE.partition_point(|&(_, hi)| hi < c);
    GO_PRINTABLE.get(i).is_some_and(|&(lo, _)| lo <= c)
}

/// The code point ranges Go's `strconv.IsPrint` accepts, found by testing
/// every code point with Go 1.26, whose tables are Unicode 15.0.0. A character
/// assigned since is escaped here because Go escapes it too.
#[rustfmt::skip]
const GO_PRINTABLE: &[(u32, u32)] = &[
    (0x0020, 0x007e), (0x00a1, 0x00ac), (0x00ae, 0x0377), (0x037a, 0x037f), (0x0384, 0x038a),
    ...
    (0x0870, 0x088e), (0x0898, 0x08e1), (0x08e3, 0x0983), (0x0985, 0x098c), (0x098f, 0x0990),
    ...
    (0x12f90, 0x12ff2), (0x13000, 0x1342f), (0x13440, 0x13455), (0x14400, 0x14646),
    ...
    (0x1bc80, 0x1bc88), (0x1bc90, 0x1bc99), (0x1bc9c, 0x1bc9f), (0x1cf00, 0x1cf2d),
    (0x1cf30, 0x1cf46), (0x1cf50, 0x1cfc3), (0x1d000, 0x1d0f5), (0x1d100, 0x1d126),
    (0x1d129, 0x1d172), (0x1d17b, 0x1d1ea), (0x1d200, 0x1d245), (0x1d2c0, 0x1d2d3),
    ...
    (0x1e2c0, 0x1e2f9), (0x1e2ff, 0x1e2ff), (0x1e4d0, 0x1e4f9), (0x1e7e0, 0x1e7e6),
...
mod tests {
    ...
    // Each expected string is what Go 1.26 prints for the same bytes with
    // fmt.Sprintf("%q", b).
    #[test]
    fn quotes_bytes_as_go_does() {
        let cases: &[(&[u8], &str)] = &[
            (b"", r#""""#),
            (
                b"plain ASCII text 0-9 ~!@#$%^&*()",
                r#""plain ASCII text 0-9 ~!@#$%^&*()""#,
            ),
            (
                b"\"quoted\" and 'single' and `back`",
                r#""\"quoted\" and 'single' and `back`""#,
            ),
            (b"back\\slash \\n literal", r#""back\\slash \\n literal""#),
            (b"a\nb\tc\rd", r#""a\nb\tc\rd""#),
            (b"\x07\x08\x0c\x0b\0", r#""\a\b\f\v\x00""#),
            (b"\x1b[31mred\x1b[0m", r#""\x1b[31mred\x1b[0m""#),
            (b"a\x7fb", r#""a\x7fb""#),
            ("a\u{9b}b".as_bytes(), r#""a\u009bb""#),
            (b"a\xffb\xc3(", r#""a\xffb\xc3(""#),
            (b"\xe2\x82 \xed\xa0\x80", r#""\xe2\x82 \xed\xa0\x80""#),
            (
                "Sponsor\u{202e}moc.live".as_bytes(),
                r#""Sponsor\u202emoc.live""#,
            ),
            ("ev\u{ad}il".as_bytes(), r#""ev\u00adil""#),
            ("CESNET\u{e0041}".as_bytes(), r#""CESNET\U000e0041""#),
            ("Paris\u{a0}CESNET".as_bytes(), r#""Paris\u00a0CESNET""#),
            ("ok \u{1f600}".as_bytes(), r#""ok 😀""#),
            ("\u{fffd}".as_bytes(), r#""�""#),
            ("a\u{2028}b".as_bytes(), r#""a\u2028b""#),
            (
                "\u{378}\u{e000}\u{ffff}".as_bytes(),
                r#""\u0378\ue000\uffff""#,
            ),
            ("a\u{3000}b".as_bytes(), r#""a\u3000b""#),
            (
        ...
        let wrong: Vec<String> = cases
            ...
            .filter_map(|&(input, want)| {
                let got = GoQuote(input).to_string();
                (got != want)
                    .then(|| format!("b\"{}\": got {got}, want {want}", input.escape_ascii()))
            })
            .collect();
